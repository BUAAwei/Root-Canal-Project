...
`;
  ...
  void main() {
    ...
    if (grayscale < 0.1) {
      gl_FragColor = vec4(texColor.rgb, 0.01); // Black or near-black pixels become semi-transparent
    } else {
      gl_FragColor = texColor; // Other pixels remain unchanged
    }
  }
`;
      for (let i = 1; i < 200; i+=5) {
        var texturePath = "/mask/" + (i)+ ".png";
